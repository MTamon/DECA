...
class Trainer(object):
    ...
    def configure_optimizers(self):
        self.opt = torch.optim.Adam(self.E_flame.parameters(), lr=self.cfg.train.lr, amsgrad=False)
    ...
    def training_step(self, batch, batch_nb):
        ...
        mask_face_eye = F.grid_sample(
            self.deca.uv_face_eye_mask.expand(batch_size, -1, -1, -1), opdict["grid"].detach(), align_corners=False
        )
        ...
        if self.cfg.model.jaw_type == "euler":
            # reg on jaw pose
            losses["reg_jawpose_roll"] = (torch.sum(codedict["euler_jaw_pose"][:, -1] ** 2) / 2) * 10.0

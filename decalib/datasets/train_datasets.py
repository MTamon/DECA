...
class TestData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.iscrop:
            if max(h, w) > 1000:
                print(
                    "image is too large, resize ", imagepath
                )  # dlib detector will be very slow if the input image size is too large
...
class EvalData(Dataset):
    def __init__(
        self, testpath, kptfolder, iscrop=True, crop_size=224, scale=1.25, face_detector="fan", face_detector_model=None
    ):
        """
        testpath: folder, imagepath_list, image path, video path
        """
